...
func DownloadReports(device ios.DeviceEntry, pattern string, targetdir string) error {
	...
	deviceConn, err := ios.ConnectToServiceWithRsd(device, crashReportCopyMobileService)
	if err != nil {
		return err
	}
...
func newMover(device ios.DeviceEntry) (*moverConnection, error) {
	deviceConn, err := ios.ConnectToServiceWithRsd(device, crashReportMoverService)
	if err != nil {
